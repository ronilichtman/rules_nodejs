...
You can install dependencies using one of these tools. `BUILD` files are generated so that Bazel can refer to those dependencies.
It contains Bazel Providers to allow interop between JS rules. 
...
2. The `@build_bazel_rules_nodejs` Bazel module is legacy, and depends on the `@rules_nodejs` module.
...
This module gives the basic ability to run Node.js programs with `nodejs_binary`, run tests with `nodejs_test`.
